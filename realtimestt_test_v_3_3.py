...
if __name__ == '__main__':
    ...
    recorder_config = {
        ...
        'audio_format': pyaudio.paInt16, # Optional: centralizing audio format
        'channels': 1,                     # Optional: centralizing channel count
        'rate': 16000                      # Optional: centralizing sample rate
    }
    ...
    def static_recording_worker():
        ...
        FORMAT = recorder_config.get('audio_format', pyaudio.paInt16)
        CHANNELS = recorder_config.get('channels', 1)
        RATE = recorder_config.get('rate', 16000)  # Sample rate
    ...
    def process_static_transcription():
        ...
        # Load the model
        model_size = recorder_config.get('model', 'Systran/faster-distil-whisper-large-v3')
        device = recorder_config.get('device', 'cuda')
        compute_type = recorder_config.get('compute_type', 'float16')
        ...
        try:
            segments, info = model.transcribe(audio_array, beam_size=recorder_config.get('beam_size', 5))
