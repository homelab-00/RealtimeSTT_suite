...
class AudioToTextRecorder:
    ...
    def __init__(self,
                 ...
                 device: str = "cuda",
                 use_main_transcription_model: bool = True,  # Added parameter
        ...
        """
        self.use_main_transcription_model = use_main_transcription_model  # Assign the parameter
        ...
        self.parent_transcription_pipe = None  # Initialize as None
        self.parent_stdout_pipe = None  # Initialize as None

        # Set device for model
        self.device = "cuda" if self.device == "cuda" and torch.cuda.is_available() else "cpu"

        # Initialize main transcription model only if use_main_transcription_model is True
        if self.use_main_transcription_model:
            ...
            # Wait for main transcription model to be ready
            ...
            logging.info("Main transcription model is disabled.")
    ...
    def transcribe(self):
        ...
        """
        if not self.use_main_transcription_model:
            logging.debug("Main transcription model is disabled.")
            return ""
        ...
        with self.transcription_lock:
            try:
                ...
                while self.transcribe_count > 0:
                    logging.debug(f"Waiting for transcription result, transcribe_count: {self.transcribe_count}")
                ...
                self._set_state("inactive")
                if status == 'success':
                    transcription = result[0]  # Get the transcription text
    ...
    def shutdown(self):
        ...
        with self.shutdown_lock:
            ...
            # Terminate the main transcription model if it was initialized
            if self.use_main_transcription_model and self.transcript_process:
