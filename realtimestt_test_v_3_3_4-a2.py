...
if __name__ == '__main__':
    ...
    # Removed Live Panel to allow console scrolling
    ...
    # New Global Variable for Typing Control
    typing_enabled = True
    ...
    def text_detected(text):
        global prev_text, rich_text_stored
        ...
        # Update the displayed_text with accumulated full_sentences and current text
        rich_text = Text()
        ...
        for i, sentence in enumerate(full_sentences):
            if i % 2 == 0:
                ...
                rich_text += Text(sentence, style="bold cyan") + Text(" ")

        # If the current text is not a sentence-ending, display it in real-time
        if text:
            rich_text += Text(text, style="bold yellow")

        # Print the accumulated rich_text to the console
    ...
    def process_text(text):
        ...
        if text:
            ...
            # Print the finalized sentence to the console with alternating colors
            index = len(full_sentences) - 1
            if index % 2 == 0:
                styled_text = Text(text, style="bold yellow")
            else:
            ...
            # Type the finalized sentence to the active window if typing is enabled
            if typing_enabled:
                try:
    ...
    # Mute the microphone at startup
    ...
    console.print("[bold cyan]F6[/bold cyan]: Disable Typing")

    # Global variables for static recording
    static_recording_active = False
    static_recording_thread = None
    static_audio_frames = []
    # live_recording_enabled = True  # Already set to False at startup
    ...
    def process_static_transcription():
        ...
        console.print("[bold green]Static Recording Transcription:[/bold green]")
    ...
    keyboard.add_hotkey('F6', disable_typing, suppress=True)
