...
pipe = pipeline(
    ...
    torch_dtype=torch_dtype,
    device=device,
    # Use sequential algorithm for better accuracy on long audio
    chunk_length_s=None
)
...
result = pipe(
    ...
    generate_kwargs={
        "language": "greek",          # Specify the language
        "task": "transcribe",         # Set the task to transcribe
        "num_beams": 5,               # Use beam search for better accuracy
        "temperature": [0.0],         # Use temperature fallback if desired
        "max_new_tokens": 448,        # Set max tokens as per model card
