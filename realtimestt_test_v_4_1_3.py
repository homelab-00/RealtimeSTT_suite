...
def correct_spelling(text, spell):
    ...
    # Log the corrections
    for i, word in enumerate(corrected_text):
        if word is None:
            print(f"Warning: Word {text.split()[i]} could not be corrected and is None.")
...
def correct_with_languagetool(text, server_url='http://localhost:8081/v2/check'):
    ...
    try:
        # Send POST request to LanguageTool server
        ...
        # Extract matches and correct the text
        matches = response_json.get('matches', [])
        corrected_text = language_tool_python.utils.correct(text, matches)
        return corrected_text

    except requests.exceptions.RequestException as e:
        print(f"Error connecting to LanguageTool server: {e}")
...
if __name__ == '__main__':
    ...
    # Optional: Check and update language_tool_python to the latest version
    try:
        import language_tool_python
        if language_tool_python.__version__ < "2.8.0":  # Replace with the latest version number if known
    ...
    # If using a local server, specify the complete URL

    # Previous instantiation which causes TypeError
    # tool = language_tool_python.LanguageTool('en-US', url='http://localhost:8081')

    # Corrected instantiation for newer versions of language_tool_python
    tool = language_tool_python.LanguageTool('en-US')
    tool._url = 'http://localhost:8081'

    # If using the public API, use the following line instead:
    # tool = language_tool_python.LanguageTool('en-US')
