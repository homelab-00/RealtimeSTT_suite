...
if __name__ == '__main__':
    ...
    typing_enabled = True  # Track whether typing is enabled
    ...
    # Variables for handling single vs double taps
    f1_press_count = 0
    f1_timer = None
    ...
    def process_text(text):
        ...
        text = text.rstrip()
        if text.endswith("..."):
            text = text[:-3]  # Remove the ellipsis
    ...
    # Added: Mute the microphone at startup
    ...
    console.print("[bold cyan]F2[/bold cyan]: Single tap to mute typing, double tap to unmute typing")
    ...
    def process_static_transcription():
        ...
        if typing_enabled:  # Check if typing is enabled
            try:
                # Release modifier keys to prevent stuck keys
                for key in ['ctrl', 'shift', 'alt', 'win']:
                    keyboard.release(key)
    ...
    def mute_typing():
        ...
        if typing_enabled:
            ...
            console.print("[bold red]Typing has been muted.[/bold red]")
        else:
            console.print("[bold yellow]Typing is already muted.[/bold yellow]")

    def unmute_typing():
        global typing_enabled
        if not typing_enabled:
            typing_enabled = True
            console.print("[bold green]Typing has been unmuted.[/bold green]")
        else:
            console.print("[bold yellow]Typing is already unmuted.[/bold yellow]")

    # Functions to handle single and double taps for F1 and F2

    def handle_F1_single_tap():
        global f1_press_count, f1_timer
        if f1_press_count == 1:
            mute_microphone()
            f1_press_count = 0
        f1_timer = None

    def handle_F1_double_tap():
        global f1_press_count, f1_timer
        unmute_microphone()
        f1_press_count = 0
        if f1_timer is not None:
            f1_timer.cancel()
            f1_timer = None

    def handle_F2_single_tap():
        global f2_press_count, f2_timer
        if f2_press_count == 1:
            mute_typing()
            f2_press_count = 0
        f2_timer = None

    def handle_F2_double_tap():
        global f2_press_count, f2_timer
        unmute_typing()
        f2_press_count = 0
        if f2_timer is not None:
            f2_timer.cancel()
            f2_timer = None
    ...
    def handle_F1_press():
        ...
        if f1_timer is not None:
            f1_timer.cancel()
        if f1_press_count == 1:
            f1_timer = threading.Timer(0.5, handle_F1_single_tap)
            f1_timer.start()
        elif f1_press_count == 2:
            handle_F1_double_tap()
    ...
    def handle_F2_press():
        ...
        if f2_timer is not None:
            f2_timer.cancel()
        if f2_press_count == 1:
            f2_timer = threading.Timer(0.5, handle_F2_single_tap)
            f2_timer.start()
        elif f2_press_count == 2:
            handle_F2_double_tap()
    ...
    keyboard.add_hotkey('F1', handle_F1_press, suppress=True)
    keyboard.add_hotkey('F2', handle_F2_press, suppress=True)
