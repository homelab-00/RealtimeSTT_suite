...
if __name__ == '__main__':
    ...
    import signal  # Import signal for potential future use
    import threading  # Import threading for inter-thread communication
    ...
    try:
        ...
        print("Failed to import 'AudioToTextRecorder'. Ensure that 'audio_recorder_v_1_0.py' is in the same directory.")
        sys.exit(1)
    ...
    if os.name == "nt" and (3, 8) <= sys.version_info < (3, 99):
        try:
            ...
            console.print("[bold red]Failed to initialize torchaudio DLL path.[/bold red]")
    ...
    def send_ctrl_c():
        """
        Signals the main thread to exit gracefully by setting the exit_event.
        """
        console.print("[bold red]Ctrl + C pressed. Preparing to exit gracefully...[/bold red]")
        exit_event.set()

    def hotkey_listener():
        keyboard.add_hotkey('ctrl+1', enable_typing)
        keyboard.add_hotkey('ctrl+2', disable_typing)
        keyboard.add_hotkey('ctrl+3', send_ctrl_c)  # Added Ctrl + 3 functionality
    ...
    try:
        while not exit_event.is_set():
            # Assuming recorder.text is a blocking call that processes audio input
            # If it's non-blocking or has a timeout, the loop can check the exit_event more frequently
            recorder.text(process_text)
    except KeyboardInterrupt:
        # Handle manual Ctrl + C if pressed directly in the terminal
        pass
    finally:
        live.stop()
        if exit_event.is_set():
            console.print("[bold red]Transcription stopped by user via Ctrl + 3. Exiting...[/bold red]")
        else:
            console.print("[bold red]Transcription stopped by KeyboardInterrupt. Exiting...[/bold red]")
        sys.exit(0)
