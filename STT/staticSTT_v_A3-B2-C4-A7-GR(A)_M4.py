...
def record_audio():
    ...
    try:
        while recording:
            ...
            # --- Silence detection additions ---
            # Measure the peak amplitude of this chunk
            peak = audioop.max(data, 2)
            # Each chunk is CHUNK samples @ RATE. Time in seconds of this chunk:
            ...
            if peak < THRESHOLD:
                ...
                # Print silence detected once
                if not silence_printed:
                    console.print("[bold yellow]Silence detected[/bold yellow]")
                    silence_printed = True

                # Keep writing up to SILENCE_LIMIT_SEC. After that, skip.
            ...
            # Write to file once per second of accumulated chunks
        ...
        # Write any leftover data in the buffer
        if buffer:
            wave_file.writeframes(b''.join(buffer))
            buffer = []
        if stream:
...
panel_content = (
    ...
    "[bold green]F3[/bold green] - Start recording | [bold green]F4[/bold green] - Stop & Transcribe"
)
