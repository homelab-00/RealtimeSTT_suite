...
if __name__ == '__main__':
    ...
    try:
        ...
        console.print(f"[bold red]Failed to import AudioToTextRecorder: {e}[/bold red]")
        sys.exit(1)
    ...
    # Initialize the recorder with exception handling
    try:
        recorder = AudioToTextRecorder(**recorder_config)
        recorder.set_microphone(False)
        console.print("[bold yellow]Live microphone is muted at startup.[/bold yellow]")
    except Exception as e:
        console.print(f"[bold red]Failed to initialize the recorder: {e}[/bold red]")
        live.stop()
        sys.exit(1)
    ...
    def transcription_loop():
        try:
            ...
            console.print("[bold yellow]Transcription loop exiting...[/bold yellow]")
    ...
    try:
        ...
        # Wait for the transcription_thread to finish
        if transcription_thread.is_alive():
            console.print("[bold green]Waiting for transcription thread to terminate...[/bold green]")
            transcription_thread.join(timeout=5)
            if transcription_thread.is_alive():
                console.print("[bold red]Transcription thread did not terminate in time.[/bold red]")
            else:
                console.print("[bold green]Transcription thread has been successfully joined.[/bold green]")
        else:
            console.print("[bold yellow]Transcription thread was not active during shutdown.[/bold yellow]")
        ...
        try:
            if hasattr(recorder, 'stop'):
                recorder.stop()
                console.print("[bold green]Recorder stopped successfully.[/bold green]")
            elif hasattr(recorder, 'close'):
                recorder.close()
                console.print("[bold green]Recorder closed successfully.[/bold green]")
        except Exception as e:
