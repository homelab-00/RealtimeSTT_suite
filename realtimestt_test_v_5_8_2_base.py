...
def stop_manual_recording(recorder):
    ...
    with manual_recording_lock:
        ...
        try:
            recorder.stop()
            # Attempt to retrieve the transcribed text
            transcribed_text = recorder.text  # Access as a property
            if callable(transcribed_text):
                transcribed_text = transcribed_text()  # If it's a method, call it
...
def main():
    ...
    global is_manual_recording  # Declare global at the beginning
    clear_console()
