...
# Import AudioToTextRecorder
# Assuming audio_recorder_v_1_0.py is in the same directory and has a class named AudioToTextRecorder
sys.path.insert(0, './')
try:
    from audio_recorder_v_1_0 import AudioToTextRecorder  # Ensure this module has stop() or close() methods
except ImportError:
    console.print("[bold red]Failed to import AudioToTextRecorder. Ensure audio_recorder_v_1_0.py is in the same directory.[/bold red]")
    sys.exit(1)
...
console.print("[bold cyan]F6[/bold cyan]: Exit Script")
...
def exit_gracefully():
    """
    Exits the script gracefully by setting the exit_event.
    """
    console.print("[bold red]F6 pressed. Exiting gracefully...[/bold red]")
    exit_event.set()
...
keyboard.add_hotkey('F6', exit_gracefully, suppress=True)
...
try:
    ...
    # Forcefully exit the script to terminate any lingering threads
    os._exit(0)
