...
def transcription_thread_function():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        recorder_transcription = AudioToTextRecorder(**recorder_config_transcription)

        # Set up global hotkeys in the main thread
        setup_hotkeys()

        try:
            recorder_transcription.start()
            while True:
                # Continuously listen and process audio
                recorder_transcription.listen()
...
def command_listener_thread_function():
    ...
    with AudioToTextRecorder(**recorder_config_command) as recorder_command:
        while True:
            recorder_command.listen()
            # You can add a short sleep to prevent high CPU usage
            time.sleep(0.1)
