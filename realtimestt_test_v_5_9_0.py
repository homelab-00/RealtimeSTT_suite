...
def capture_audio_to_buffer():
    """Captures live audio from the microphone and stores it in the buffer."""
    p = pyaudio.PyAudio()
    try:
        # Open stream
...
def feed_audio_from_buffer(recorder):
    """Feeds audio chunks from the buffer into the transcription engine."""
    while True:
...
def main():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        recorder_config = {
            ...
            'realtime_model_type': 'tiny.en',  # Use the tiny.en model for real-time transcription
            'on_realtime_transcription_update': lambda text: text_detected(text, recorder), 
            'silero_deactivity_detection': True,
            'allowed_latency_limit': 100,  # Increased latency limit
