...
if __name__ == '__main__':
    ...
    # Function to install the 'rich' library if not present
    def install_rich():
        subprocess.check_call([sys.executable, "-m", "pip", "install", "rich"])

    # Attempt to import 'rich'; install if not available
    try:
        import rich
    except ImportError:
    ...
    # Import other required libraries (assumed to be installed)
    ...
    # Handle Windows-specific DLL path initialization for torchaudio
    if os.name == "nt" and (3, 8) <= sys.version_info < (3, 99):
        try:
            from torchaudio._extension.utils import _init_dll_path
            _init_dll_path()
        except ImportError:
            print("[bold red]Failed to initialize DLL path for torchaudio. Ensure torchaudio is installed correctly.[/bold red]")
    ...
    # Function to clear the console (unused but kept for reference)
    def clear_console():
        os.system('clear' if os.name == 'posix' else 'cls')

    # Function to preprocess detected text
    ...
    # Function to handle detected text in real-time
    ...
    # Function to process finalized text
    ...
    # Worker function to handle static recording
    ...
    # Function to start static recording
    ...
    # Function to stop static recording
    def stop_static_recording():
        ...
        try:
            ...
            console.print("[bold red]faster_whisper is not installed. Please install it using 'pip install faster-whisper' to use static transcription.[/bold red]")
            return
    ...
    try:
        # Start the recorder (assuming it has a 'start' method that runs the recording loop)
        recorder.start()
