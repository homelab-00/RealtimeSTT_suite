...
def configure_logging():
    ...
    file_handler.setLevel(logging.DEBUG)  # Capture all DEBUG and above logs
    ...
    # Add more external libraries as needed

    # Prevent external libraries from propagating to the root logger
    logging.getLogger('ctranslate2').propagate = False
    logging.getLogger('faster_whisper').propagate = False
    logging.getLogger('pyautogui').propagate = False

    # Optional: Redirect `stderr` to the logger to capture external warnings/errors
    ...
    stream = StreamToLogger(stderr_logger, logging.ERROR)
    sys.stderr = stream
...
# === 3. Proceed with Other Imports After Logging Configuration ===
