...
if __name__ == '__main__':
    ...
    is_static_recording = False  # Flag to track static recording mode
    ...
    def start_static_recording():
        global is_static_recording, recorder
        if is_static_recording:
            console.print("[bold yellow]Static recording is already in progress.[/bold yellow]")
        else:
            is_static_recording = True
            recorder.start_recording_on_voice_activity = False
            recorder.stop_recording_on_voice_deactivity = False
            recorder.is_recording = False  # Ensure not recording before starting
            recorder.start()
            console.print("[bold green]Static recording started.[/bold green]")

    def stop_static_recording():
        global is_static_recording, recorder
        if not is_static_recording:
            console.print("[bold yellow]Static recording is not in progress.[/bold yellow]")
        else:
            is_static_recording = False
            recorder.stop()
            console.print("[bold green]Static recording stopped. Processing...[/bold green]")
            transcription = recorder.transcribe()
            process_text(transcription)
            # Reset the recorder to automatic mode
    ...
    def hotkey_listener():
        ...
        keyboard.add_hotkey('ctrl+3', start_static_recording)
        keyboard.add_hotkey('ctrl+4', stop_static_recording)
