...
if __name__ == '__main__':
    ...
    try:
        ...
        user_input = input("This demo needs the 'pynput' library, which is not installed.\nDo you want to install it now? (y/n): ")
        if user_input.lower() == 'y':
            try:
                install_pynput()
                from pynput import keyboard as pynput_keyboard
                ...
                print(f"An error occurred while installing 'pynput': {e}")
                sys.exit(1)
        else:
            print("The program requires the 'pynput' library to run. Exiting...")
            sys.exit(1)
