# integrated_speech_to_text_correction.py
...
def correct_spelling(text, spell):
    ...
    for word in text.split():
        ...
        if correction is None:
            print(f"Warning: Word '{word}' could not be corrected and is retained.")
...
# Function to correct grammatical errors using T5 model
def correct_grammar(text, tokenizer, model):
    # Prepend the task prefix if required by the model
    input_text = "gec: " + text
...
# Function to perform additional corrections using LanguageTool
...
# --- Integration into Speech-to-Text Script ---
...
if __name__ == '__main__':
    ...
    # Removed the incorrect version check for language_tool_python
    # If version checking is necessary, use importlib.metadata as shown below:
    """
    try:
        import language_tool_python
        import importlib.metadata
        try:
            lt_version = importlib.metadata.version('language_tool_python')
            if lt_version < "2.8.0":
                console.print("[bold yellow]Updating 'language-tool-python' to the latest version...[/bold yellow]")
                subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "language-tool-python"])
                import language_tool_python
                console.print("[bold green]'language-tool_python' successfully updated.[/bold green]")
        except importlib.metadata.PackageNotFoundError:
            console.print("[bold red]'language_tool_python' package not found during version check.[/bold red]")
    except Exception as e:
        console.print(f"[bold red]Failed to update 'language_tool_python': {e}[/bold red]")
    """

    # Load the grammatical correction model
    console.print("Loading grammatical correction model...")
    tokenizer = AutoTokenizer.from_pretrained("vennify/t5-base-grammar-correction")
    model = AutoModelForSeq2SeqLM.from_pretrained("vennify/t5-base-grammar-correction")
    model.eval()

    # Initialize the spell checker
    console.print("Initializing spell checker...")
    spell = SpellChecker(language='en')  # Specify language if necessary

    # Initialize LanguageTool
    console.print("Initializing LanguageTool...")
    try:
        # Correct Initialization: Use 'remote_server_uri' as a separate parameter
        tool = language_tool_python.LanguageTool('en-US', remote_server_uri='http://localhost:8081')
    except Exception as e:
        console.print(f"[bold red]Failed to initialize LanguageTool: {e}[/bold red]")
        sys.exit(1)

    # --- Read and Corrected Output Handling ---
    ...
    def text_detected(text):
        ...
        if text:
            # Apply correction pipeline here
            corrected = correct_spelling(text, spell)
            corrected = correct_grammar(corrected, tokenizer, model)
            corrected = correct_with_languagetool(corrected, tool)

            rich_text += Text(corrected, style="bold yellow")
    ...
    def process_text(text):
        ...
        # Apply correction pipeline to the finalized sentence
        corrected = correct_spelling(text, spell)
        corrected = correct_grammar(corrected, tokenizer, model)
        corrected = correct_with_languagetool(corrected, tool)

        full_sentences.append(corrected)
        prev_text = ""
        text_detected("")
    ...
    recorder_config = {
        'spinner': False,
        'model': 'Systran/faster-distil-whisper-large-v3',  # or other models
        'input_device_index': 1,
        'realtime_model_type': 'tiny.en',  # or other types
        'language': 'en',
        ...
        'device': 'cuda',          # Ensure CUDA is available if using GPU
        'compute_type': 'float16'  # Ensure compatibility with your GPU
    }
    ...
    try:
        ...
        console.print(f"[bold red]Failed to initialize AudioToTextRecorder: {e}[/bold red]")
        sys.exit(1)
    ...
    def process_static_transcription():
        ...
        try:
            segments, info = model_whisper.transcribe(audio_array, beam_size=recorder_config['beam_size'])
        ...
        if not transcription:
            console.print("[bold yellow]No transcription was generated from the static recording.[/bold yellow]")
            return

        # Apply correction pipeline to the transcription
        corrected = correct_spelling(transcription, spell)
        corrected = correct_grammar(corrected, tokenizer, model)
        corrected = correct_with_languagetool(corrected, tool)

        # Display the transcription
        console.print("Static Recording Transcription:")
        console.print(f"[bold cyan]{corrected}[/bold cyan]")
    ...
    try:
        ...
        try:
            ...
            console.print(f"[bold red]Error closing LanguageTool: {e}[/bold red]")
